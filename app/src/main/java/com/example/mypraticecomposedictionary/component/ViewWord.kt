...
)
{
    Box(modifier = Modifier.border(BorderStroke(2.dp, color = Color.Black))) {
        ...
        Column(
            ...
            Row {
                val titleSize = dimensionResource(R.dimen.title_size).value.sp
                Text(
                    text = data.slug,
            ...
            {
                Text(text = "\t-\t${sense.partsOfSpeech.joinToString(", ")}")
                Text(text = "\t\t>\t${sense.englishDefinitions.joinToString(", ")}")
            }
            val formString = stringResource(R.string.forms_)
            Text(text = formString)
            for (japanese in data.japanese)
            {
                Text(text = "\t-\t${japanese.word} (${japanese.reading})")
            }
        }
    }
}